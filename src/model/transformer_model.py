...
class WhisperClassifier(nn.Module):
    def __init__(self):
        ...
        self.flatten = nn.Sequential(
            nn.Conv1d(self.n_ctx, 1, kernel_size=1),
        )

        self.classifer = nn.Sequential(
            nn.Linear(self.n_states, 10),
            nn.Softmax(),
        )

    def forward(self, mel: torch.Tensor) -> Dict[str, torch.Tensor]:
        # TODO: add music classification layers
        encoded = self.encoder(mel)
        flattened = self.flatten(encoded).squeeze()
        classes = self.classifer(flattened)
